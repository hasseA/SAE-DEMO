...
# -- Unrestricted local provider usage -------------------------------------


def test_repeated_inference_calls_beyond_twenty_are_allowed(
    client: TestClient, mock_provider: _FakeProviderRecorder
) -> None:
    responses = []
    for _ in range(4):
        responses.extend(_run_full_scenario(client, "greenhouse", memory_mode="off"))

    assert len(mock_provider.calls) == 28
...
def test_removed_quota_environment_variables_have_no_effect(
    client: TestClient,
    mock_provider: _FakeProviderRecorder,
    configured_memory_artifact,
    monkeypatch: pytest.MonkeyPatch,
    ...
    assert comparison["status"] == "ready"
    assert len(mock_provider.calls) == 14
    status = client.get("/api/status").json()
    assert "usage_protection_enabled" not in status


def test_multiple_full_comparisons_run_in_one_process_without_local_quota(
    client: TestClient,
    mock_provider: _FakeProviderRecorder,
    configured_memory_artifact,
) -> None:
    first, _, _ = _build_ready_comparison(client, "greenhouse", "off")
    second, _, _ = _build_ready_comparison(client, "new_studio", "on")
...
def test_application_sets_no_quota_session_cookie(client: TestClient) -> None:
    health = client.get("/api/health")
    root = client.get("/")

    assert "sae_demo_session" not in client.cookies
    assert "set-cookie" not in health.headers
    assert "set-cookie" not in root.headers


def test_frontend_has_no_password_or_usage_limit_ui(client: TestClient) -> None:
    html = client.get("/").text
    js = client.get("/static/app.js").text

    assert "Demo access code" not in html
    assert 'type="password"' not in html
    assert "usage limit" not in html.lower()
    assert "usage limit" not in js.lower()
