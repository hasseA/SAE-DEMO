...
Configure the provider (required for any run — see "Nebius/NVIDIA provider setup" below) and, only if you want to try Memory ON, point `SAE_DEMO_MEMORY_FILE` at a local, gitignored artifact envelope (for example, one already prepared under `.local/memory/`):

```
export NEBIUS_API_KEY=...           # required to start any run
export SAE_DEMO_MEMORY_FILE=.local/memory/<name>.json   # optional; only needed for Memory ON
```
