...
`sae_demo/web_app.py` is a minimal FastAPI application that serves the static frontend (`sae_demo/static/`) and exposes `/api/health` and `/api/status`. It is intentionally the smallest possible shell: `/api/status` reports only public-safe fields (application name, stage label, backend status, the public target-model identifier, whether a provider API key is *present* as a boolean, and static "not active in M5A" feature labels) — never the key's value, `.env` contents, filesystem paths, or any memory/private-SAE data. Provider calls, Emotional Memory loading, and scenario execution are not wired to this web UI in M5A.

Install dependencies (adds `fastapi` and `uvicorn` to the existing `openai`/`python-dotenv` set):

```
pip install -r requirements-dev.txt
```

Start the local server (binds to localhost only):

```
python -m uvicorn sae_demo.web_app:app --host 127.0.0.1 --port 8000
```

Then open `http://127.0.0.1:8000` in a browser. The page checks `/api/health` and `/api/status` on load and shows a connected/error indicator; the Scenario, Memory OFF/ON, Conversation, and Comparison areas are placeholders labeled as coming in later M5 stages.
