...
Then open `http://127.0.0.1:8000` in a browser. The page checks `/api/health` and `/api/status` on load and shows a connected/error indicator; picking a scenario and clicking "Start Scenario" begins an in-memory run, and "Next Segment" advances it one segment at a time to a completed transcript. The Memory OFF/ON, Conversation, and Comparison areas remain placeholders labeled as coming in later M5 stages.
