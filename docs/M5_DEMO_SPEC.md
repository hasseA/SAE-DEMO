...
Status: **design document; M5A, M5B, M5C, M5D, M5E, M5F, and M5G implemented.** M5A (Section 20) — the minimal FastAPI shell, `/api/health`, `/api/status`, and the static vanilla frontend (`sae_demo/web_app.py`, `sae_demo/static/`) — is implemented. M5B — the scenario-run web flow (`/api/scenarios`, `/api/runs`, `/api/runs/{run_id}`, `/api/runs/{run_id}/advance`, and the scenario-run frontend) wiring the existing, unchanged `ScenarioEngine` into the web app for `frozen`-mode runs of the two built-in fixtures — is implemented. M5C — Memory OFF/ON integration — is implemented: `/api/runs` now accepts a validated `memory_mode` (`off`/`on`, fixed for a run's lifetime), and each "advance" sends the next segment through the existing, unchanged `NebiusProvider`, opaque `memory_loader`, and M4B `DEFAULT_BEHAVIORAL_USE_POLICY`, returning a real assistant response. Memory ON resolves exactly one operator-configured artifact via the `SAE_DEMO_MEMORY_FILE` environment variable — never a hardcoded path, and never a profile/network choice exposed in the UI — and its payload is never returned in any API response. To avoid a second implementation of memory-placement/policy semantics, the web app drives `CompatibilityRunner.build_history()`/`send_turn()`, the same methods `CompatibilityRunner.run()` itself now uses internally. M5D — the controlled comparison view — is implemented: `POST /api/runs/{run_id}/alternate` creates a fresh, opposite-memory-mode run of the same scenario from an already-completed run (built by the same `_start_run_entry` construction path as any other run, so it never reuses the first run's history), and `GET /api/comparisons/{comparison_id}` returns the paired state — `"pending"` until both runs finish, `"ready"` (with the two transcripts aligned by segment) only once both complete successfully, `"failed"` if either does. A comparison always pairs exactly one completed Memory OFF run and one completed Memory ON run of the same scenario; it is never assembled from two unrelated runs or presented as complete while incomplete. No automated scoring, sentiment analysis, or "which response is better" judgment is performed anywhere in M5D — the comparison is the two transcripts, for a human to read. M5E — the conceptual Emotional Memory visualization and Experiment 8 evidence card (Sections 10 and 11) — is implemented as static, public-safe UI content only, added to `sae_demo/static/index.html`/`styles.css`: a small, clearly-labeled "Emotional Memory — conceptual view" diagram (synthetic step sequences only, never derived from or exposing the real memory artifact, its internals, or any live model state) and a compact evidence card carrying the one approved Experiment 8 sentence plus simple metadata (3 providers, 3 conditions, 9 sessions). No new backend endpoint or logic was added for M5E; both render identically whether or not a provider or memory artifact is configured, and the controlled comparison (M5D) remains the visually dominant part of the page. M5F — the Scenario Wizard / Bring Your Own Story workflow — is implemented in a new `sae_demo/custom_scenario.py` module plus additions to `sae_demo/web_app.py`/`static/`: a local, deterministic prompt generator (`POST /api/scenario-wizard/prompt`, no provider or network call) turns a few plain-language ingredients into a copyable prompt for an AI the user chooses; the resulting seven-section `[ROLE_NAME]`-bracketed story is parsed and validated (`POST /api/custom-scenarios`, merging Part C's parse step with draft creation — an invalid paste creates nothing and returns its full validation report instead), reviewed/edited (`PATCH /api/custom-scenarios/{id}`, refused once frozen), and explicitly frozen (`POST /api/custom-scenarios/{id}/freeze`, requiring all seven roles present and non-empty). A frozen custom scenario is resolved by the exact same `_start_run_entry`/`_resolve_scenario` path a built-in scenario already used — no second run or comparison engine was built for it — so it runs through the unmodified M5D controlled Memory OFF/ON comparison, replaying its exact frozen text unchanged on both sides. Custom scenario drafts live only in a process-local, in-memory registry (`_CUSTOM_SCENARIO_REGISTRY`, governed by the same lock as the run/comparison registries already used) — never written to disk, cleared on server restart, exactly like every other run/comparison registry in this app. Automatic Nebius scenario generation, live mid-run ("exploratory mode") editing, deployment, and public Git publication remain unimplemented / not performed. M5G — demo hardening and submission readiness — is implemented as a hardening-only pass: no new scenario, memory, or comparison machinery was added. It resolves the per-turn completion-token budget (`max_tokens`) from one configurable, environment-backed function (`compatibility_runner.resolve_max_tokens`, default raised from 200 to 400) instead of a bare constant, still applied identically to Memory OFF and Memory ON at the single shared `_start_run_entry` call site — this reduces live responses ending mid-sentence without introducing any condition-specific asymmetry. `NebiusProvider.complete()` now treats a structurally malformed (but HTTP-successful) response the same as a failed request — a safe `NebiusProviderError`, never an unhandled exception. The frontend gained a short "How to try it" section, a visible "Waiting for model..." state (plus a disabled-button guard) for the one action that makes a real provider call, wording distinguishing scenario preparation from the controlled comparison, and a purely visual OFF/ON accent on the comparison columns — assistant/segment text continues to render as plain text via `.textContent` with whitespace preserved (`white-space: pre-line`), never as Markdown or raw HTML. See `docs/DEMO_READINESS_CHECKLIST.md` for the operational pre-submission checklist this stage added. No frontend, backend, or API code beyond M5A/M5B/M5C/M5D/M5E/M5F/M5G and what M3A–M3D/M4A/M4B already built (`sae_demo/scenario.py`, `scenario_engine.py`, `nebius_provider.py`, `config.py`, `compatibility_runner.py`, `memory_loader.py`) exists yet. This document specifies the smallest compelling demo that can be built around that existing backend without redesigning SAE or expanding scope beyond what a hackathon judge needs to see. It builds directly on the frozen boundary in `docs/decisions/SAE_DEMO_M4_CONSUMPTION_BOUNDARY_FREEZE.md` and does not reopen it.
...
**Note on stage naming (added post-hoc, this original list left unchanged above):** as actually implemented, the label "M5F" was used for the Scenario Wizard / Bring Your Own Story workflow instead of the "polish/demo hardening" milestone originally planned here, and that hardening work was implemented one stage later, as "M5G" — see the top status line for what M5F and M5G each actually shipped. This section's original A-F milestone list is left as first written rather than renumbered after the fact.
